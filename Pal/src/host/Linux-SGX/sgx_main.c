...
#ifdef ASAN
/*
 * HACK: `setup_asan` is not called inside `main`, but defined as a constructor with a reserved
 * priority of 0. This is because we need to run it before module constructors generated by ASan
 * itself (which have a priority of 1).
 *
 * Note that this is necessary only because we're in an executable linked against Glibc. In other
 * cases, we invoke the constructors directly, so we have full control over initialization.
 */
