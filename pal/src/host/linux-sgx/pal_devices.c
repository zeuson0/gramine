...
static int dev_map(PAL_HANDLE handle, void** addr, pal_prot_flags_t prot, uint64_t offset,
    ...
    if (mem < g_pal_public_state.shared_address_start
            || (uintptr_t)mem + size > (uintptr_t)g_pal_public_state.shared_address_end) {
...
int _PalDeviceIoControl(PAL_HANDLE handle, uint32_t cmd, unsigned long arg, int* out_ret) {
    ...
    int ioctl_ret = ocall_ioctl(handle?handle->sock.fd:0, cmd, (unsigned long)untrusted_addr);
    if (ioctl_ret < 0) {
