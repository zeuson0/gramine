...
void* libos_syscall_mmap(void* addr, size_t length, int prot, int flags, int fd,
    ...
    if (flags & MAP_SHARED && hdl && !strcmp(hdl->fs->name, "shm")) {
        ...
        memory_range_start = g_pal_public_state->user_address_start;
        memory_range_end = g_pal_public_state->user_address_end;
    }
    if (flags & (MAP_FIXED | MAP_FIXED_NOREPLACE)) {
        ...
        if (ret < 0) {
            ...
            if (memory_range_start == g_pal_public_state->user_address_start) {
